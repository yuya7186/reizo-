    ...
    if request.method == 'POST':
        ...
        if file and file.filename:
            ...
            try:
                ...
                prompt = (
                    'これは日本のスーパーマーケットの買い物レシートの写真です。\n\n'
                    '【作業手順】\n'
                    '1. まずレシートに印字されているすべての文字をOCRとして正確に読み取る\n'
                    '2. 読み取った商品名が略称や誤読の場合、一般的な日本語の正式な食品名に補正する\n'
                    '   （例：「ﾆﾎﾝｼｭ」→「日本酒」、「ｶｯﾌﾟﾗ」→「カップラーメン」）\n'
                    '3. 食品・飲料・食材・調味料だけを抽出する\n\n'
                    '【除外するもの】\n'
                    '- 袋代、ポイント、割引、税、合計などの金額行\n'
                    '- 日用品、洗剤、シャンプー、雑貨など食品以外\n'
                    '- レシートのヘッダー・フッター（店名・日付・レジ番号など）\n\n'
                    '【出力フォーマット】品名,個数（1行1品目、カンマ区切り、余計な文章なし）\n\n'
                    '【補正例】\n'
                    'セイロン抹茶,1\n牛乳,2\n卵,1\nキャベツ,1\nアリナミンドリンク,1\nジンジャーエール,1\n醤油,1'
                )
